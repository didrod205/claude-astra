...
Three Claude skills for work that happens outside a text editor: building 3D
scenes you can walk, driving native desktop apps, and verifying a front end by
...
automate. The scripts are zero-dependency Node (22+) driving headless Chrome over
the DevTools protocol; nothing to install.
...
Each one is built around a verification loop, because in all three domains the
...
GPU. `desktop-app-driver` is macOS-only and needs the computer-use tools.
